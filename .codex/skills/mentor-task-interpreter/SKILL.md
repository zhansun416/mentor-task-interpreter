...
2. **Restore context.** Build a chronological timeline using timestamps when available; otherwise retain source order and mark timing as uncertain. Identify the active project, prior decisions, deliverables, changes of mind, and any date that can safely anchor relative time.
3. **Extract instructions.** Capture action verbs, scope, acceptance signals, dates, prohibitions, and conditional language. Apply [instruction priority](references/instruction-priority.md) before treating a speaker as authoritative. Resolve pronouns and phrases such as “that figure” or “the previous version” only when their antecedent is evidenced. Link each statement to evidence.
...
6. **Produce the package.** Fill `task-spec.json` against [the schema](references/task-spec.schema.json), including `execution_order` only for evidence-backed, confirmed sequencing. Keep speculative ordering in `inferences`. Preserve relative deadline wording and add an absolute date only when the record supplies a reliable anchor. Then render a human-readable brief from [task-brief.md](templates/task-brief.md) and a ready-to-send handoff from [codex-handoff.md](templates/codex-handoff.md). Follow [evidence rules](references/evidence-rules.md).
